...
- **무성 영상 인코딩이 99.9%에서 멈춤** - 1.2.1에서 주입한 `anoisesrc`가 무한 스트림이라 앞/뒤 공백 경로에서 ffmpeg가 EOF에 도달하지 못해 마지막에 정지하던 문제. 이제 소스 길이만큼 `-t`로 잘라 종료가 보장됨
- **더미 오디오 노이즈 감소** - 주입 노이즈 진폭을 `0.001` → `0.00001`(약 -100 dB)로 낮춰, 디코더는 인식하지만 사람 귀에는 사실상 무음

### 추가

- **인코딩 로그 UI 표시** - 실패한 파일마다 "자세한 로그 보기" 토글로 ffmpeg 명령어와 전체 stderr를 확인 가능. 각 영역마다 복사 버튼 제공
- **사이드카 `.log` 파일** - 실패한 작업은 출력 경로 옆에 `<output>.mkv.log` 파일로 전체 로그를 저장. 앱 재시작 후에도 원인 추적 가능

## [1.2.1] - 2026-05-25

### 수정

- **무성 영상 인코딩 실패** - 오디오 트랙이 없는 영상(카메라 직출 `.mov` 등)에서 앞/뒤 공백 옵션 사용 시 `[0:a]` 참조가 실패하던 문제 해결
  - 추가 ffprobe 없이 수집된 `AudioCodec` 정보로 오디오 유무 판별
  - 오디오가 없으면 매우 작은 화이트노이즈(`anoisesrc amplitude=0.001`, 약 -60 dB)를 자동 주입해 출력에 항상 오디오 스트림이 존재하도록 보장 — SyncLauper 동기화 재생 일관성 유지

## [1.2.0] - 2026-05-25

### 추가

- **비디오 회전 옵션** - 출력 영상을 90°, 180°, 270°(시계방향) 회전. 큐의 모든 파일에 일괄 적용되며, 셀렉터는 앞/뒤 공백 옵션 옆에 위치
  - 90°/270° 회전 시 가로·세로가 뒤바뀌므로 출력 해상도와 HEVC level이 자동 보정
  - 앞/뒤 공백 프레임도 회전 후 크기에 맞춰 생성되어 concat 정합성 유지
